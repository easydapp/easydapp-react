...
> => {
    return Promise.all([
        (upper_query_combined ?? proxy_query_combined)(dapp.combined).then((combined) => {
            if (!combined) throw new Error(`combined not found: ${dapp.combined}`);
            return combined;
        }),
        (upper_query_publisher ?? proxy_query_publisher)(dapp.publisher).then((user) => {
        ...
        Promise.all(
            (dapp.metadata?.combined_anchors ?? []).map(async (combined_anchor) => {
                return (upper_query_combined ?? proxy_query_combined)(combined_anchor).then(
                    (combined) => {
